...
**実測**（2026-09-22 / 実キー / **費用は推定ではなく実請求額**）:
4方式 × 3条件 × 2試行 = **24試行すべて合格** → [evaluation/results.md](evaluation/results.md)

T-04 複合障害:

| 方式 | 成功 | 平均所要 | 平均LLM呼出 | 平均費用（実請求額） | 成功1件あたり |
|---|---|---|---|---|---|
| A 高性能モデル固定（`gpt-5`） | 2/2 | 133.8s | 5.0 | $0.083353 | $0.083353 |
| **B 製品のルーティング（既定）** | 2/2 | 134.8s | 6.5 | **$0.001134** | **$0.001134** |
| B' 自前フォールバック | 2/2 | **79.5s** | 12.0 | $0.004696 | $0.004696 |
| R 固定ランブック（LLM不使用） | 2/2 | 39.2s | 1.0 | $0.000174 | $0.000174 |

**いちばん言いたいこと**: A（`gpt-5` 固定）の費用は B の **73.5倍**。
しかも**所要時間は 0.99倍でほぼ同じ**、判断回数も 5.0 対 6.5 呼出で大差ない。
高性能モデルの「手数の少なさ」は単価差をまったく埋め合わせなかった。

**2つめ**: 同じ「安いモデルを使う」でも、製品のルーティングに委ねる（B）と
自前で候補列を回す（B'）で差が出た。**B は B' の約4.1分の1の費用**、
代わりに **B' は 1.7倍速い**。安さを取るか速さを取るかの選択になる。

**3つめ（自律性の証拠でもある）**: B が実際に使ったモデルは試行ごとに変わった
（`gpt-oss-120b` / `qwen3.7-flash` / `gpt-5.6-luna`）。
モデルを1つに固定していないことが、そのまま記録に残る。

R は最安だが、想定済みの障害しか扱えない（判断が決定木に固定）ので同じ土俵ではない。

**§12.2 の固定条件**（「AIが余計なことをしない」の実測）:

| 条件 | 合格条件 | 結果 |
|---|---|---|
| C-0 正常時 | **不要な変更を提案しない** | 全 8 試行合格（変更案 0件で `RESOLVED`） |
| C-1 主回線断のみ | 変更せず残存課題として引き継ぐ | 全 8 試行合格（適用 0件・残存課題を起票） |

| 仕組み | 実装 |
|---|---|
| 候補列を設定で宣言（モデル名を直書きしない、M-17） | `backend/app/llm/route_policy.yaml` |
| 方式切替 | `NW_ROUTE_VARIANT=a\|b\|bs` → `route_policy.py` の `resolve()` |
| 費用の算出 | **表示・評価は実請求額**（`GET /v1/generation` の `total_cost`）。呼出前の予算判定のみ単価表 |
| 単価非公開モデルの扱い | 0円扱いにすると上限が無限になるため、保守的な上振れ単価で見積る（`budget_cost_usd`） |
| 費用上限（N-04） | 案件 0.50 USD / 構成図PDF 0.20 USD の**2本立て**。上限到達**前**に呼出停止 |
| 再現手順 | `cd backend && uv run python scripts/eval_abr.py --trials 2 --conditions T04,C0,C1` |
| 候補モデルの検証 | `uv run python scripts/bench_models.py` → [evaluation/models.md](evaluation/models.md) |

**採用の根拠も実測**: 候補15モデル × 2タスクを実タスクで検証したところ、
**単価が最安のモデルは構造化出力に不合格**（200を返すのにスキーマに適合しない＝
課金されるのに使えない）、**旧設定の第一候補は構成図の接続を5本中4本しか読めなかった**。
合格したモデルだけを候補列と画面の選択肢に載せている。
...
- 自動テスト **38本** green（`test_approval` 9 / `test_model_select` 8 /
  ...
  `test_send_gate` 4）
- 方式比較 **24試行すべて合格**（4方式 × 3条件 × 2試行）、NEEDS_HUMAN率 0%。
  条件ごとに合格基準を変えている（復旧できること／**余計なことをしないこと**／
  変更せず引き継ぐこと）
- 録画再生のみ（`NW_ROUTE_MODE=mock`）で AI モードが完走。外部通信なし・費用$0
...
- 同じ複合障害でも**手数が変わる**: `gpt-5` は4〜6呼出、Named Router 経由は5〜8呼出、
  `gpt-5.4-nano` は12呼出。どれも同じ結論（予備回線ルータの誤ルール削除）へ到達し、
  T-04 の8試行すべて復旧まで完走した
- ツール実行回数も方式で異なる（A 13.0回 / B 14.5回 / B' 27.0回 / R 15.0回）
  ＝**手順が固定でない証拠**
- **同じ方式でも試行ごとに経路が違う**: B の2試行は 8呼出/16ツール と 5呼出/13ツール。
  観測結果に応じて次の手が決まっている
- 条件が変われば手数も変わる。正常時（C-0）は 2〜10呼出で「変更不要」に到達し、
  **変更案を1件も出さなかった**（余計なことをしない、の実測）
...
**実測**（2026-09-22 / 実キー）: Named Router 経由（`orcarouter/fusion-flash` →
`qwen/qwen3.7-flash`）で、PDFから5機器・5接続を抽出し登録構成と全件一致
（`comparison.ok = true`・$0.000211）。攻撃PDFでも抽出は正しいまま。
